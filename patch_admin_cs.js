...
const srcDir = path.join(__dirname, 'src', 'views', 'AdminPage');
const csDir = path.join(srcDir, 'cs');
if (!fs.existsSync(csDir)) {
    fs.mkdirSync(csDir, { recursive: true });
}

const loginPath = path.join(csDir, 'CSLogin.js');
const dashboardPath = path.join(csDir, 'CSDashboard.js');
...
const dashboardCode = `import React, { useState, useEffect, useRef } from 'react';
import {
  Box, Flex, VStack, HStack, Heading, Text, Badge, Button, Input, IconButton,
  Avatar, Divider, useToast, Spinner
...
console.log('CS Dashboard created');
