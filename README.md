...
('Luas Wilayah', '5.34 km²', 'FaMap', 'green.500'),
...
-- TABEL PESAN PENGADUAN (Chat)
CREATE TABLE complaint_messages (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  complaint_id TEXT REFERENCES complaints(id) ON DELETE CASCADE,
  sender_type TEXT NOT NULL, -- 'user' atau 'admin'
  message TEXT,
  image_url TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE comments (
  ...
  parent_id BIGINT REFERENCES comments(id) ON DELETE CASCADE, -- Untuk reply bertingkat
  name TEXT NOT NULL,
  email TEXT,
  content TEXT NOT NULL,
...
-- TABEL POPUP NOTIFIKASI
CREATE TABLE popups (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  title TEXT,
  content TEXT, -- Teks atau URL Gambar
  type TEXT DEFAULT 'text', -- 'text' atau 'image'
  button_label TEXT,
  button_link TEXT,
  is_active BOOLEAN DEFAULT TRUE,
  created_at TIMESTAMPTZ DEFAULT NOW()
...
CREATE TABLE dusuns (
  ...
  map_link TEXT, -- Link Google Maps Embed
  image_url TEXT,
  color TEXT DEFAULT 'brand.500',
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE public_services (
  ...
  title TEXT NOT NULL,
  description TEXT,
  icon_name TEXT, -- Nama icon (contoh: FaInfo)
  link TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
