...
const NgawonggoLogo = ({ color = "accent.green", fontSize = "xl", iconSize = 8, showText = true, flexDirection = "row" }) => {
  ...
  const hasNoBg = settings?.background_image === 'none' || settings?.background_image === '';
  const bgImage = hasNoBg ? 'none' : (settings?.background_image || "url('https://images.unsplash.com/photo-1542361345-89e58247f2d5?q=80&w=2070&auto=format&fit=crop')");
  const bgColor = hasNoBg ? 'transparent' : (settings?.background_color || "brand.600");
  const glow = hasNoBg ? 'none' : (settings?.glow_color || "rgba(255,255,255,0.2)");
  ...
  return (
    <Flex align="center" direction={flexDirection}>
      ...
      <MotionBox
        ...
        bgColor={bgColor}
        bgImage={hasNoBg ? 'none' : (bgImage.startsWith('http') || bgImage.startsWith('/') ? `url('${bgImage}')` : bgImage)}
        bgSize="cover"
        bgPosition="center"
        position="relative"
        overflow="hidden"
        borderColor="whiteAlpha.300"
        borderWidth={hasNoBg || settings?.border_style === 'none' ? "0px" : "1px"}
        border={hasNoBg ? 'none' : borderStyle}
        boxShadow={hasNoBg || glow === 'none' ? 'none' : `0 4px 15px ${glow}`}
        animate={controls}
      >
        {!hasNoBg && (
          <Box
